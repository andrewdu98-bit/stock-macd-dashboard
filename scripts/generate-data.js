...
const SELL_THE_NEWS_URL = 'https://sellthenews.org/wsb?lang=zh&truthShowAll=1';
const COMPANY = {SPY:'SPDR S&P 500 ETF Trust',TSLA:'Tesla, Inc.',NVDA:'NVIDIA Corporation',GM:'General Motors Company',MSFT:'Microsoft Corporation',GOOG:'Alphabet Inc.',AVGO:'Broadcom Inc.',IAU:'iShares Gold Trust',MU:'Micron Technology, Inc.',IBIT:'iShares Bitcoin Trust ETF',AMAT:'Applied Materials, Inc.',GLW:'Corning Incorporated',COHR:'Coherent Corp.',ORCL:'Oracle Corporation',SNDK:'SanDisk Corporation',AAPL:'Apple Inc.',AMD:'Advanced Micro Devices, Inc.',META:'Meta Platforms, Inc.',SOXL:'Direxion Daily Semiconductor Bull 3X Shares',INTC:'Intel Corporation',BE:'Bloom Energy Corporation',EWY:'iShares MSCI South Korea ETF',BRT:'BRT Apartments Corp.',AXTI:'AXT, Inc.',LITE:'Lumentum Holdings Inc.',AAOI:'Applied Optoelectronics, Inc.',AMZN:'Amazon.com, Inc.',JPM:'JPMorgan Chase & Co.',QQQ:'Invesco QQQ Trust',SOFI:'SoFi Technologies, Inc.',UUUU:'Energy Fuels Inc.',ALAB:'Astera Labs, Inc.',ASTS:'AST SpaceMobile, Inc.',BB:'BlackBerry Limited',FLY:'Flywire Corporation',FRO:'Frontline plc',HYLN:'Hyliion Holdings Corp.',LPTH:'LightPath Technologies, Inc.',LUNR:'Intuitive Machines, Inc.',NOK:'Nokia Oyj',NVTS:'Navitas Semiconductor Corporation',QBTS:'D-Wave Quantum Inc.',QUBT:'Quantum Computing Inc.',RDDT:'Reddit, Inc.',RGTI:'Rigetti Computing, Inc.',SPCX:'SPAC and New Issue ETF',USO:'United States Oil Fund, LP',XLE:'Energy Select Sector SPDR Fund'};
const TAG_GROUPS = {
  '算力':['NVDA','AMD','AVGO','ARM','ALAB','ANET','CIEN','SMCI'],
  '服务器':['DELL','HPE','SMCI','ANET','CIEN','CSCO','NTAP','HPQ'],
  '能源':['CEG','VST','BE','UUUU','NEE','DUK','SO','EXC','AES','AEP','D','DTE','EIX','ES','ETR','EVRG','FE','PCG'],
  '石油':['USO','XLE','XOM','CVX','COP','OXY','FRO','EOG','FANG','DVN','HAL','BKR','MPC','PSX','APA','CTRA','BNO','SLB'],
  '半导体内存':['NVDA','AMD','AVGO','MU','SNDK','WDC','AMAT','ASML','ADI','INTC','QCOM','MRVL','NXPI','MCHP','MU','AXTI','COHR','LITE','AAOI','FORM','NVTS'],
  '量子计算':['IONQ','QBTS','RGTI','QUBT','LPTH'],
  '太空卫星':['ASTS','RKLB','LUNR','SATS','SPCX'],
...
function emaPullbackSignal(rows, emaLine, period){const n=rows.length-1;if(n<2||!Number.isFinite(emaLine[n])||!Number.isFinite(emaLine[n-1]))return{hit:false,period};const close=rows.map(r=>r.close),low=rows.map(r=>r.low);const rising=emaLine[n]>emaLine[n-1];const priorAbove=close[n-1]>emaLine[n-1];const touched=low[n]<=emaLine[n];const closedBackAbove=close[n]>=emaLine[n];const nearLine=Math.abs(close[n]-emaLine[n])/emaLine[n]<=0.03;const hit=!!(rising&&priorAbove&&touched&&closedBackAbove&&nearLine);return{hit,period,ema:+emaLine[n].toFixed(4),low:+low[n].toFixed(4),close:+close[n].toFixed(4),badge:`回踩${period}EMA`,label:hit?`回踩${period}EMA`:'未回踩'}}
...
function decodeHtml(text){return String(text||'').replace(/\\n/g,'\n').replace(/\\t/g,'\t').replace(/&nbsp;/g,' ').replace(/&amp;/g,'&').replace(/&lt;/g,'<').replace(/&gt;/g,'>').replace(/&#39;/g,"'").replace(/&quot;/g,'"').replace(/&#(\d+);/g,(_,n)=>String.fromCharCode(Number(n))).replace(/&#x([0-9a-f]+);/gi,(_,n)=>String.fromCharCode(parseInt(n,16)))}
function htmlToLines(html){const text=String(html||'').replace(/<script[\s\S]*?<\/script>/gi,'\n').replace(/<style[\s\S]*?<\/style>/gi,'\n').replace(/<\/(p|div|li|h[1-6]|tr|section|article|header|footer|nav|table|blockquote|pre|br)>/gi,'\n').replace(/<(li|p|div|h[1-6]|tr|section|article|header|footer|nav|table|blockquote|pre)[^>]*>/gi,'\n').replace(/<[^>]+>/g,' ');return decodeHtml(text).split(/\n+/).map(s=>s.replace(/\s+/g,' ').trim()).filter(Boolean)}
function extractSectionHtml(html, startHeading, endHeading){const start=html.indexOf(startHeading);if(start<0)return'';const end=endHeading?html.indexOf(endHeading,start):html.length;return html.slice(start,end>start?end:html.length).trim()}
const translationCache=new Map();
async function translateText(text){const key=String(text||'').trim();if(!key)return'';if(translationCache.has(key))return translationCache.get(key);const url='https://translate.googleapis.com/translate_a/single?client=gtx&sl=en&tl=zh-CN&dt=t&q='+encodeURIComponent(key);const r=await fetch(url,{headers:{'user-agent':'Mozilla/5.0'}});if(!r.ok)throw new Error(`translate ${r.status}`);const j=await r.json();const out=(j?.[0]||[]).map(part=>part?.[0]||'').join('')||key;translationCache.set(key,out);return out}
async function translateOverviewSection(html){const section=extractSectionHtml(html,'<h2>1. Overall Market Overview and Sentiment</h2>','<h2>2. Core Themes/Topics</h2>');if(!section)return'';let out=section.replace('<h2>1. Overall Market Overview and Sentiment</h2>',`<h2>1. ${await translateText('Overall Market Overview and Sentiment')}</h2>`);const ps=[...out.matchAll(/<p>([\s\S]*?)<\/p>/gi)];for(const m of ps){const plain=decodeHtml(m[1].replace(/<[^>]+>/g,' ').replace(/\s+/g,' ').trim());out=out.replace(m[0],`<p>${await translateText(plain)}</p>`)}return out}
async function translateStockSection(html){const section=extractSectionHtml(html,'<h2>5. Individual Stock Sentiment Analysis</h2>','<h2>6. Key Risk Signals and Macro/Policy Signals</h2>');if(!section)return'';let out=section.replace('<h2>5. Individual Stock Sentiment Analysis</h2>',`<h2>5. ${await translateText('Individual Stock Sentiment Analysis')}</h2>`);const rows=[...out.matchAll(/<tr>([\s\S]*?)<\/tr>/gi)];for(const row of rows){const cells=[...row[1].matchAll(/<t[hd]>([\s\S]*?)<\/t[hd]>/gi)].map(m=>m[1]);if(!cells.length)continue;const isHeader=row[0].includes('<th>');const values=[];for(let i=0;i<cells.length;i++){const txt=decodeHtml(cells[i].replace(/<[^>]+>/g,' ').replace(/\s+/g,' ').trim());values.push(isHeader?await translateText(txt):(i===0||i===1?txt:await translateText(txt)))}const tag=isHeader?'th':'td';const rebuilt=`<tr>${values.map(v=>`<${tag}>${v}</${tag}>`).join('')}</tr>`;out=out.replace(row[0],rebuilt)}return out}
async function fetchSellThenNews(){try{const r=await fetch(SELL_THE_NEWS_URL,{headers:{'user-agent':'Mozilla/5.0','accept-language':'zh-CN,zh;q=0.9,en;q=0.8'}});if(!r.ok)throw new Error(`SellTheNews ${r.status}`);const html=await r.text();const title='WSB 情绪分析';const description=await translateText((html.match(/<meta[^>]+name=["']description["'][^>]+content=["']([^"']*)["']/i)||[])[1]||'');const lines=htmlToLines(html);const topLine=lines.find(l=>l.includes('Top discussed tickers:'))||'';const topTickers=topLine.includes(':')?topLine.split(':').slice(1).join(':').split(',').map(s=>s.trim()).filter(Boolean).filter((x,i,a)=>a.indexOf(x)===i):[];const overview=await translateOverviewSection(html);const stocks=await translateStockSection(html);return{sourceUrl:SELL_THE_NEWS_URL,fetchedAt:new Date().toISOString(),title,description,topTickers,sections:{overview,stocks},content:''}}catch(e){return{sourceUrl:SELL_THE_NEWS_URL,fetchedAt:new Date().toISOString(),error:e.message||String(e)}}}
